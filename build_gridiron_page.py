...
EXTRA_CSS = """
    ...
    /* 2.3: reference-grade table. Dense like Pro Football Reference, sortable
       from the header, the sorted column marked; everything else is the page
       as it was. */
    thead th { padding: 8px 9px; font-size: 10.5px; cursor: pointer; user-select: none; border-right: 1px solid rgba(29, 64, 48, .35) }
    thead th:first-child { cursor: default }
    thead th:hover { color: var(--chalk) }
    thead th.sort-desc, thead th.sort-asc { color: var(--chalk); box-shadow: inset 0 -2px 0 var(--accent) }
    thead th.sort-desc::after { content: " BC"; font-size: 8px; color: var(--accent) }
    thead th.sort-asc::after { content: " B2"; font-size: 8px; color: var(--accent) }
    tbody td { padding: 6px 9px; font-size: 12.5px; border-right: 1px solid rgba(29, 64, 48, .25) }
    tbody td.name { font-size: 14px }
    tbody td.sorted { background: rgba(255, 210, 63, .06) }
    tbody tr:nth-child(even) td { background-color: rgba(255, 255, 255, .012) }
    tbody tr:nth-child(even) td.sorted { background-color: rgba(255, 210, 63, .08) }
    tbody tr:hover td { background-color: rgba(255, 210, 63, .07) }
    tbody img, tbody .avatar { width: 24px; height: 24px; border-radius: 50%; object-fit: cover; vertical-align: middle; background: var(--line) }
    .team, .pos { font-family: 'IBM Plex Mono', monospace }
    .chip.extra { display: none }
    .examples.open .chip.extra { display: inline-block }
    .chip.more { color: var(--accent); border-style: solid }
    .tot-bar { display: none }
"""
