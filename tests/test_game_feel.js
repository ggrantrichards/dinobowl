...
// normalize line endings so source-text assertions survive CRLF checkouts
const GAME = fs.readFileSync(path.join(__dirname, "..", "static", "game", "game.js"), "utf8").replace(/\r\n/g, "\n");
...
(async () => {
  ...
  if (stalledRunner) {
    ...
    g.playT = 15.99;
    // The AA presentation pass gives deep catches an impactMoment beat; the
    // freeze left over from the catch scene above must not steal this staged
    // step (LESSON #9: slow-mo steals test time).
  ...
  for (const kind of ["tackle", "firstdown", "catch", "interception"]) {
    ...
    const semantic = kind === "tackle"
      ? scene.dive && scene.wrap && scene.diveAt < scene.wrapAt && scene.wrapAt - scene.diveAt >= 0.15 &&
