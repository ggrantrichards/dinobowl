...
(async () => {
  ...
  key("Enter");            // opening possession: kickoff BEAT (1s banner) → spot at the 25
  ...
  noErr("pregame → opening possession");
  ...
  // Owner decision: play-call CARD SCREENS are the default (a Dino Bowl
  // improvement over Retro Bowl); fast flow is the opt-in setting.
  forceDrive("B");
  dbg.enterPlaycall();
  stepFor(0.3);
  check("defense: card screen is the default flow", g.state === "defcall", g.state);
  key("1");
  check("defense: presnap", g.state === "presnap", g.state);
  const shell0 = g.defCall;
  key("e");   // the CHANGE PLAY chip / Q/E still cycles at the line
  check("defense: change-play chip cycles the shell at the line", g.defCall !== shell0 && g.state === "presnap",
    (shell0 && shell0.name) + " -> " + (g.defCall && g.defCall.name));
  // fast flow (opt-in) lines up immediately with a shell picked
  g.coachMode = false;
  g.state = "dead"; g.deadT = 0; g.deadNext = null;
  forceDrive("B");
  dbg.enterPlaycall(); stepFor(0.3);
  check("defense: fast-flow setting lines up instantly with a shell",
    g.state === "presnap" && !!g.defCall, g.state + "/" + (g.defCall && g.defCall.name));
  g.coachMode = true;
  ...
  // ---- #2 peanut punch: works, timing-based
  {
    ...
    // deterministic weather: with Math.random pinned low below, a RAIN puddle
    // check would otherwise stagger every player and starve the recovery test
    g.weather.type = "CLEAR"; g.weather.wind = { x: 0, y: 0 }; g.weather.fumbleMod = 0;
  ...
  //      (probabilistic in-game; verified by inspection + fumble bump test)
  {
    ...
    key(" ");
    g.weather.type = "CLEAR"; g.weather.wind = { x: 0, y: 0 };   // no puddle slips under a pinned RNG
    stepFor(0.3);
  ...
  {
    // halftime shows are OFF by default now (owner: Retro Bowl parity) —
    // this test exercises the setting-ON path
    g.halftimeShow = true;
